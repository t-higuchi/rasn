#[test]
fn test_ber_encode_decode_tagged_type() {
    ...
    // decode test
    let type1_de = rasn::ber::decode::<Type1>(&type1_ber).unwrap();
    let type2_de = rasn::ber::decode::<Type2>(&type2_ber).unwrap();
    let type3_de = rasn::ber::decode::<Type3>(&type3_ber).unwrap();
    let type4_de = rasn::ber::decode::<Type4>(&type4_ber).unwrap();
    let type5_de = rasn::ber::decode::<Type5>(&type5_ber).unwrap();

    assert_eq!(type1, type1_de);
    assert_eq!(type2, type2_de);
    assert_eq!(type3, type3_de);
    assert_eq!(type4, type4_de);
    assert_eq!(type5, type5_de);
}
